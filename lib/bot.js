...
class Bot {
  ...
  constructor(config, addons) {
    ...
    this.controller = Botkit.slackbot({
      clientSigningSecret: this.config.slack.clientSigningSecret,
    });
  ...
  connect() {
    ...
    this.controller.setupWebserver(process.env.PORT || 3001, (err, webserver) => {
      if (err) {
      ...
      this.controller.createOauthEndpoints(webserver);
      this.controller.createWebhookEndpoints(webserver, this.bot, (error) => {
        if (error) {
          logger.error('Error starting webhooks!', err);
        }
      });
    });
